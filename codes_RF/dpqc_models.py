...
def unet_model(input_shape, initializer, kernel_size=3, activation='relu'):
    ...
    from tensorflow.keras.initializers import GlorotUniform, Ones, Zeros, RandomNormal, he_normal, he_uniform
    ...
    # dense3 = layers.Dense(8, activation='relu', kernel_initializer=initializer)(dense2)
    # dense4 = layers.Dense(4, activation='relu', kernel_initializer=initializer)(dense3)
...
def load_best_model(key):
    ...
    path = "DATA/DEEP_QC/"
    all = os.listdir(path)
    all = [[i, float(i.split("_")[1])] for i in all if key in i]
...
class DeepQC():
    ...
    def add_noise(self, noise_factor=0.01):
        """
        Augment the data.
    ...
    def add_shift(self, shift_factor=40):
        # add noise to the data
        train_input = self.train_data_np
        train_input_good = np.array([i[0] for i in train_input if i[1] == 1 and i[0].shape[0] == 426])[:,1:-1] # good data
        train_input_good = np.expand_dims(train_input_good, axis=2)                                     # add a dimension to the data
        train_label_good = np.array([i[1] for i in train_input if i[1] == 1]).reshape(-1,1) # good data

        for index, line in enumerate(train_input_good):
            shift = np.random.randint(-shift_factor, shift_factor)
            line = np.roll(line, shift)
            train_input_good[index] = line
        # concatenate the original data and the shifted data
        self.train_input = np.concatenate((self.train_input, train_input_good), axis=0)
        self.train_label = np.concatenate((self.train_label, train_label_good), axis=0)
        #print the shape of the data
        print(f"the folowing are the dataset accessable:\n train_input: {self.train_input.shape}\n train_label: {self.train_label.shape}\n test_input: {self.test_input.shape}\n test_label: {self.test_label.shape}")


    def add_scaling(self, scaling_factor=0.5):
        # add noise to the data
        train_input = self.train_data_np
        train_input_good = np.array([i[0] for i in train_input if i[1] == 1 and i[0].shape[0] == 426])[:,1:-1]
        train_input_good = np.expand_dims(train_input_good, axis=2)                                     # add a dimension to the data
        train_label_good = np.array([i[1] for i in train_input if i[1] == 1]).reshape(-1,1) # good data
        ...
        for index, line in enumerate(train_input_good):
            ...
            line = line * scale
            train_input_good[index] = line
        # concatenate the original data and the scaled data
        self.train_input = np.concatenate((self.train_input, train_input_good), axis=0)
        self.train_label = np.concatenate((self.train_label, train_label_good), axis=0)
        #print the shape of the data
        print(f"the folowing are the dataset accessable:\n train_input: {self.train_input.shape}\n train_label: {self.train_label.shape}\n test_input: {self.test_input.shape}\n test_label: {self.test_label.shape}")
    ...
    def train(self, initializer = "zeros", model_type="unet", kernel_size = 3, epochs=10, batch_size=32, verbose=1, lr=0.01,
        ...
        if plot_history and self.history.history["val_accuracy"][-1] > 0.85:
            ...
            self.model.save(f"DATA/DEEP_QC/{model_type}_{round(self.history.history['val_accuracy'][-1],3)}_{initializer}_kernelsize_{kernel_size}_lr_{lr}.h5", save_format='h5')
...
class CustomCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        current_accuracy = logs.get('val_f1_score')  # Replace with 'acc' if using older Keras versions
