...
if [ $param -eq 4 ]; then
    ...
    OUTPUT_FILE="gmt_temp/plot_depth_$title.ps"   # Output file name (PostScript format)
    ...
    gmt makecpt -Cjet -T25,28,31,34,37,40,43,46,49
    ...
    # Plot the data using GMT
    gmt begin $OUTPUT_FILE
    gmt makecpt -Cjet -T1.6,1.75,1.9,2.06
    gmt coast $REGION $PROJECTION -B -Gchocolate -Sazure2
    ...
    gmt plot ani_temp.txt -Sv0.3i -Gyellow -W2p -Baf
    # gmt colorbar -C -Dx8c/2c+w12c/0.5c+jTC+h -Bxaf+l$title -By+lkm
    awk 'BEGIN {FS=","} NR>1 {print $3, $2, $1}' hk-hd.csv > new_temp.txt
    gmt text new_temp.txt -F+f6p,Helvetica+jLM -Dj0.3c/0.1c -Gwhite
    gmt end
fi
